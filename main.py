...
#         lr_retireval = optimizer.state_dict()['param_groups'][0]['lr']
#         lr_reuse = optimizer.state_dict()['param_groups'][1]['lr']
#         logger.info(f'lr_retireval\t:{lr_retireval}\t lr_reuse\t:{lr_reuse}\t')
        lr = optimizer.state_dict()['param_groups'][0]['lr']
        logger.info(f'lr\t:{lr}\t ')

#         get updated sim_case_id  
        
        if config.ususe_rerank and epoch>1:
        ...
        while True:
            ...
            if config.ususe_rerank and not flast_train_topk is None:  
                batch = process_rerank(train_data, batch, flast_train_topk, config)
                    
#             if config.ususe_rerank:
#                 last_train_topk = open(os.path.join(config.saved_path, 'id', '{}_train_topk.json'.format(epoch-1)), 'r')
#                 last_val_topk = open(os.path.join(config.saved_path, 'id', '{}_val_topk.json'.format(epoch-1)), 'r')
#                 last_test_topk = open(os.path.join(config.saved_path, 'id', '{}_test_topk.json'.format(epoch-1)), 'r')
#                 flast_train_topk = json.load(last_train_topk)
...
#         # test_loss = validate(model, test_data, config, batch_cnt)
#         score = p3_val
#         if epoch >= config.warmup and config.save_model:
#             cur_time = datetime.now().strftime("%Y-%m-%d %H-%M-%S")
#             logger.info('*** Model Saved with overall score = {}, at {}. ***'.format(score, cur_time))
...
#     ipdb.set_trace()
    ...
    while True:
        ...
        case_out, topk_caseids_out, candidate_scores, candidate_caseids = model.get_similarity(train_caseid2didtid, train_didtid2caseid, dialogues, batch, train_vecs, topk=(1,10), num_cluster_candidates=num_cluster_candidates)
        # move from GPU to CPU
        case_out = case_out.cpu().data.numpy()
        ...
        if th.is_tensor(topk_caseids_out):
            topk_caseids_out = topk_caseids_out.cpu().data.tolist() # bs, K=10  前10个与给定case最相似的case的id
        else :
            topk_caseids_out = topk_caseids_out.data.tolist() # bs, K=10  前10个与给定case最相似的case的id
