...
#         #reuse
#         train_ctx_path = CASE_DATA_PATH + '/bs_context_train.txt' if config.ctx_with_bs else CASE_DATA_PATH + '/context_train.txt'
...
#                 load current text and img vector
                text = mode_texts[caseid].strip('\n')#根据caseid取当前对话文本，有believe state
...
#                 # load topk paired text and img vector, and corresponding caseid
#                 topk_texts, topk_imgvecs, topk_caseids = [], [], []
#                 for k, paired in enumerate(pairs):#检索相似样本
#                     if k >= K:
#                         break
#                     _did, _tid = paired['did2'], str(paired['tid2'])#相似样本的did，tid
                    
#                     _caseid = train_didtid2caseid[_did][_tid]#为什么从训练样本中取caseid
#                     topk_caseids.append(_caseid)
# #                     ipdb.set_trace()
#                     _text = train_texts[_caseid].strip('\n')#取相似样本文本
#                     topk_texts.append(_text)
        
#                     _turn_info = dialogues[_did]['dialogue'][int(_tid) - 1]#取当前样当前轮次对话
#                     _imgs = _turn_info['user']['imgs'] + _turn_info['agent']['imgs']
#                     _imgvec = np.zeros(self.config.dim_img_in, dtype=float)
#                     if len(_imgs) != 0:
#                         _img_name = _imgs[0]
#                         _imgvecid = train_imgname2vecid[_img_name]
#                         _imgvec = train_imgvecs[_imgvecid]
#                         print(f'1 {_did}, {_tid}, {_imgvecid}, {_imgvec}')
#                     topk_imgvecs.append(_imgvec) 
#                     print(f'1 {_did}, {_tid}, {_imgvec}')
                    
 

                curr_ctx_id = self._sent2id(self.tokenize(mode_ctx_data[caseid].strip('\n')))#tokenizer + 映射为id
                curr_rsp = self._sent2id(self.tokenize(mode_rsp_data[caseid].strip('\n')))
...
#                     ipdb.set_trace()
                    ...
                    sim_rsp = self._sent2id(self.tokenize(train_rsp_data[sim_caseid].strip('\n')))
...
#                     text = text,#文本
                    imgvec = imgvec,
#                     topk_texts = topk_texts, # list of str
#                     topk_imgvecs = topk_imgvecs, # list of np array, K * dim_img_in
                    topk_caseids = topk_caseids,  # list of int, K
                    
                    # list of 文本tokenizer后的id
                    curr_ctx_id = curr_ctx_id,
                    curr_rsp=curr_rsp,
#                     sim_ctxs=sim_ctxs,  # list of list id, k * seq len
                    sim_ctxs_id = sim_ctxs_id,
                    sim_rsps=sim_rsps,  # list of list id, k * seq len
                    sim_ids = sim_ids,
                    sim_imgs = sim_imgs,
                    max_sim_ctx_len=max_sim_ctx_len,
                    max_sim_rsp_len=max_sim_rsp_len
                )
#                 ipdb.set_trace()
